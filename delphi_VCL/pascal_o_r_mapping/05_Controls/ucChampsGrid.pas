...
uses
  ...
  {$IFDEF FPC}
    LCLType, Types,
  {$ENDIF}
  SysUtils, Classes, VCL.Controls, DB, VCL.Dialogs, VCL.StdCtrls, VCL.Grids, VCL.Buttons;
...
type
 ...
 =
  ...
  public
    constructor Create( _Owner: TChampsGrid;
                        _OnEditButtonClick: TNotifyEvent;
                        _OnGetPickListItems: TOnGetPickListItems); reintroduce;
...
begin
     ...
     cg                := _Owner;
     OnEditButtonClick := _OnEditButtonClick;
     OnGetPickListItems:= _OnGetPickListItems;
     C:= nil;
end;

destructor TInplaceEdit_Champ.Destroy;
begin

     inherited;
end;

procedure TInplaceEdit_Champ.UpdateContents;
var
   NewC: TChamp;
begin
     inherited;
     NewC := cg.Champ_courant;
     if NewC <> C
     then
         begin
         C:= NewC;
         if Assigned( C)
         then
             C.InplaceEditUpdateContents( Self);
         end;
end;

procedure TInplaceEdit_Champ.Change;
begin
     inherited;
     if Assigned( C)
     then
         if C <> cg.Champ_courant
         then
             begin
             C.InplaceEditDecroche;
             C:= nil;
             end
         else
             C.InplaceEditChange;
...
procedure TInplaceEdit_Champ.KeyDown(var Key: Word; Shift: TShiftState);
begin
     if Assigned( C)
     then
         begin
         C.InplaceEditKeyDown( Key, Shift);
         if   (C.Definition.fcbChamp = nil)
            or(not C.Definition.fcbChamp.Visible)
         then
             inherited;
         end
     else
...
begin
     ...
     then
         begin
         Champ:= Champs.Champ_from_Field( ChampDefinition.Nom);
         if Assigned( Champ)
         then
             begin
...
procedure TChampsGrid.SetEditText(ACol, ARow: Integer; const Value: string);
var
   Champs: TChamps;
   Champ: TChamp;
...
begin
     Result:= ExtractFilePath(uForms_EXE_Name)+'etc\'+NamePath(Self)+'.txt';
end;
...
procedure TChampsGrid.InplaceEditListGetPickListItems(ACol, ARow: Integer;
  Items: TStrings);
begin
