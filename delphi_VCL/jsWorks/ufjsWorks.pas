...
var
  ...
  var
     ...
     S: String;
  begin
       NomFichier:= IncludeTrailingPathDelimiter( Repertoire)+'index.html';
       if FileExists( NomFichier)
       then
           begin
           S:= String_from_File( NomFichier);
           HTTP_Interface.Send_HTML( S);
           uLog.Log.PrintLn( 'Envoi racine ');
           end
       else
           begin
           HTTP_Interface.Send_Not_found;
           uLog.Log.PrintLn( '#### Fichier non trouvé :'#13#10+uri);
           end;
  end;
  ...
  var
     ...
     S: String;
  begin
       NomFichier:= IncludeTrailingPathDelimiter( Repertoire)+StringReplace(uri, '/', '\', [rfReplaceAll]);
       if FileExists( NomFichier)
       then
           begin
           uLog.Log.PrintLn( 'Envoi fichier '#13#10+uri);
