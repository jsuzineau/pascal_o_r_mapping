...
begin
     ...
     then                    //on ouvre simplement le TBufDataSet  qui y est
...
begin
     ...
     do
       ...
       then
           ...
           else // normalement on n'arrive ici qu'avec les TBufDataSet 
               DBDataSets[ID]:= nil;
           ...
           (*if D is TBufDataSet 
           then
               if not Assigned( TBufDataSet (D).OnReconcileError)
               then
                   TBufDataSet (D).OnReconcileError
                   :=
     ...
     do
       ...
       then
           if     (TBufDataSet (D).ChangeCount   > 0       )
              and (TBufDataSet (D).ProviderName <> sys_Vide)
           then
               begin
               if ModeDEBUG_1
               then
                   fClientDataset_Delta.Execute( TBufDataSet (D));
