...
begin
     try
        //Pour éviter l'appel de Positionne alors que les attributs ne sont
        //pas créés
...
var
   Memo_Top: Integer;
   procedure _SetBounds( _C: TControl; _Left, _Top, _Width, _Height: Integer);
   //pour éviter l'exception 'TControl.ChangeBounds loop detected'
   begin
        if     (_C.Left  = _Left  )
           and (_C.Top   = _Top   )
           and (_C.Width = _Width )
           and (_C.Height= _Height)
        then
            exit;
        _C.SetBounds( _Left, _Top, _Width, _Height);
   end;
begin
     if Creation_en_cours then exit;

     Button.Left:= ClientWidth -2 -ButtonWidth;
     Button.Top := 0;

     Memo_Top:= Button.Top+Button.Height;
     _SetBounds( Memo, Marge, Memo_Top,
                       ClientWidth -2*Marge, ClientHeight-Marge-Memo_Top);
end;
