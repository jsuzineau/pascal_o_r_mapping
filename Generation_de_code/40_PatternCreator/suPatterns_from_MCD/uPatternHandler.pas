...
type
 ...
 =
  ...
  public
    ...
    procedure Produit( _RepertoirePaquet: String);
  //déboguage
  private
    Log_Actif: Boolean;
    slLog: TStringList;
...
procedure TPatternHandler.Produit( _RepertoirePaquet: String);
var
   CibleName: String;
   RepertoirePaquet: String;
begin
     RepertoirePaquet:= RepertoireCible+_RepertoirePaquet;
     ForceDirectories( RepertoirePaquet);
     slLog.Add( 'avant produit '+ExtractFileName( Source));
     slLog.SaveToFile( RepertoirePaquet+ExtractFileName( Source)+'.log');
     Log_Actif:= True;
     CibleName:= RemplaceParametres( ExtractFileName( Source));

     Log_Actif:= True;
     slCible.Text:= RemplaceParametres( slSource.Text);
     slCible.SaveToFile( RepertoireCible + _RepertoirePaquet + CibleName);
     Log_Actif:= False;
end;
